...
public class AncesBox extends AncesBoxParent
{
	...
	public void calcCoords(PresetChartOptions options, ChartMaker maker, int curGen, StylingBoxScheme stylingBoxes, OpgSession session)
	{
		...
		double determinedHeight = rootIsDescendantStyle?stylingBoxes.getEffectiveRootDescendantHeight((PortraitChartMaker)maker)/2.0:boxHeight/2.0 + styleBox.paddingAmount;
		
		//calculate first element of upper bounds array for tight fit
		getUpperBounds().add(new Double(determinedHeight));
		//calculate first element of lower bounds array for tight fit
		getLowerBounds().add(new Double( -(determinedHeight + vertSeperation)));
		...
		if (curGen < maxGen)
		{
			...
			{
				//This is where you determine and add the duplicate label
...
//					}
			...
			else
			{
				...
				if (styleBox.isIntruding && options.isAllowIntrusion()/* && !rootIsDescendantStyle*/){
					double remainingAncesIntrudeWidth = styleBox.intrudeWidth - (stylingBoxes.getAncesStyle(gen).getBoxWidth() + stylingBoxes.getAncesStyle(gen).getRelativeOffset());
					StylingBox rootDescendant = stylingBoxes.getDescStyle(0);
					double remainingDescIntrudeWidth = rootDescendant.intrudeWidth - (rootDescendant.getBoxWidth() + rootDescendant.rootBackOffset);
					
					double remainingIntrudeWidth = rootIsDescendantStyle?remainingDescIntrudeWidth:remainingAncesIntrudeWidth;
...
//				}
			...
			if(options.isAllowIntrusion() && styleBox.isIntruding){
				...
				this.getUpperBounds().set(0,getUpperBounds().get(0)+intersectUpperHeight);
				intersectLowerHeight = (motherVOffset - getLowerBounds().get(0)) - 3*styleBox.borderlineWidth;
				this.getLowerBounds().set(0, getLowerBounds().get(0)+intersectLowerHeight);
			}
	...
	void drawAncesRootTree(ChartMargins chart, PortraitChartOptions options, double x, double y, OpgSession session)
	{
	...
	public void drawAncesTreeRec(int curGen, ChartMargins chart, PresetChartOptions options, double x, double y, OpgSession session)
	{
		...
		if (!(curGen == 0 && options.getAncesGens() > 0 && options.getDescGens() > 0) || (options.isIncludeSpouses() && options.getDescGens() > 0))
				drawBox(chart, options, chart.xOffset(x), chart.yOffset(y), myBoxWidth, (upperBoxBound - lowerBoxBound), chartIndex, session);
		...
		else
		{
			...
			{
				Font arrowFont = opgOptions.getFont().getFont(Font.PLAIN, (float)styleBox.endLineArrowFontSize);
				...
				double shaftLength = determineArrowLength(styleBox);
				
				
				chart.addDrawCommand(new DrawCmdMoveTo(chart.xOffset(x + xOffset), chart.yOffset(y)));
				chart.addDrawCommand(new DrawCmdEndChartArrow(styleBox.endLineArrowShaftHeight, styleBox.endLineArrowHeadHeight, styleBox.endLineArrowHeadLength, shaftLength, styleBox.borderlineWidth, Color.black, session.getBaseOptions().getAncesScheme().getColor(getIndi().id)));
				chart.addDrawCommand(new DrawCmdSetFont(arrowFont, Color.black));
				chart.addDrawCommand(new DrawCmdMoveTo(chart.xOffset(x + xOffset), chart.yOffset(y + (upperBoxBound - (spaceDistance+arrowFontHeight)))));
	...
	private double determineArrowLength(StylingBox styleBox){
		double shaftLength = styleBox.endLineArrowShaftLength;
		final double incrementAmount = 5;
		
		double incrementLength = shaftLength / incrementAmount;
		
		int numGens = 0;
		
		if ((getIndi().father != null) && (getIndi().mother == null))
			numGens = father.maxGensInTree;
		else if ((mother != null) && (father == null))
			numGens = mother.maxGensInTree;
		else
			numGens = Math.max(mother.maxGensInTree, father.maxGensInTree);
		
		if(numGens >= incrementAmount)
			return shaftLength;
		else
			return numGens*incrementLength;
