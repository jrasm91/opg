...
public class BoxLayout extends BoxLayoutParent{
	...
	public boolean canFit(Individual indi, double width, double height, LandscapeChartOptions ops, double fontSize, String dupLabel ){
		//TODO THIS IS NO LONGER USED
		return true;
	}
	
	public void drawNormalLines(OpgSession session, ChartMargins chart, PresetChartOptions options, 
			StylingBox style, double x, double y, double width, double totalHeight, double textPos,
			Individual indi, LineLayout line,
			String dupLabel, boolean maleDescendant, Rectangle2D.Double boxCoords, String chartIndex, boolean single){
		double horizTextGap = style.textMargin;
		double linepos = 0.0;
		TextDirection direction;
		if (rotateSingleDescendants && single)
			direction = TextDirection.NINETY;
		else
		...
		if(direction == StylingBox.TextDirection.NORMAL)
		{
			...
			line.draw(session, chart, style, options, x + horizTextGap, linepos, width - (horizTextGap* 2), totalHeight, indi, dupLabel, maleDescendant, 0, chartIndex, direction);

		}
		else if (direction == StylingBox.TextDirection.NINETY)
		{
			linepos = x + width - textPos;
			line.draw(session, chart, style, options, linepos, y + (totalHeight / 2.0) - horizTextGap, totalHeight - (horizTextGap* 2), width, indi, dupLabel, maleDescendant, 0, chartIndex, direction);

		}
		else if (direction == StylingBox.TextDirection.ONE_EIGHTY)
		{
			linepos = y - (totalHeight / 2.0) + textPos;
			line.draw(session, chart, style, options, x + width - horizTextGap, linepos, width - (horizTextGap* 2), totalHeight, indi, dupLabel, maleDescendant, 0, chartIndex, direction);
		}
		else
		{
			linepos = x + textPos;
	...
	public void drawMarriageLines(OpgSession session, ChartMargins chart, PresetChartOptions options,
			StylingBox style, double x, double y, double width, double totalHeight, double textPos,
		Individual indi, LineLayout line,
	...
	public void drawAnces(ChartMargins chart, PresetChartOptions options, StylingBox style,
			...
			Rectangle2D.Double boxCoords, String chartIndex, OpgSession session){
		//Draw your box;
		totalHeight = hgt;
		...
		if (options.isRoundedCorners())
			chart.addDrawCommand(new DrawCmdRoundRect(x-width/2.0, y, width,totalHeight, lineWidth, style.cornerCurve, Color.BLACK, session.getBaseOptions().getAncesScheme().getColor(indi.id), boxCoords));
		else
			chart.addDrawCommand(new DrawCmdFillRect(x-width/2.0, y, width,totalHeight, lineWidth, Color.BLACK, session.getBaseOptions().getAncesScheme().getColor(indi.id), boxCoords));
		...
		for(int i = 0; i < lines.size(); i++){
			...
			drawNormalLines(session, chart, options, style, x-width/2.0, y+totalHeight/2.0, width, totalHeight, textPos, indi, line, dupLabel, true, boxCoords, chartIndex, false);
		...
		if (style.weddingLayout.lines.size() > 0){
			...
			if((indi.gender == Gender.MALE && style.weddingDisplayType == StylingBox.WeddingPositions.HUSBAND_POSTFIX) ||
				...
				for (int i = 0; i < style.weddingLayout.lines.size(); i++){
					...
					drawMarriageLines(session, chart, options, style, x-width/2.0, y+totalHeight/2.0, width, totalHeight, textPos, indi, line, dupLabel, boxCoords, 0);
	...
	public void drawDescSingle(ChartMargins chart, StylingBox style, 
		...
		for(int i = 0; i < lines.size(); i++){
			...
			drawNormalLines(session, chart, options, style, x, y, width, totalHeight, textPos, indi, line, "", true, boxCoords, "", true);
	...
	public void drawDescMarried(ChartMargins chart, StylingBox style, PresetChartOptions options, 
		...
		for(int i = 0; i < lines.size(); i++){
			LineLayout line = lines.get(i);
			double textPos = (i<style.textPositions.size()?style.textPositions.get(i):style.textPositions.get(style.textPositions.size() - 1));
			drawNormalLines(session, chart, options, style, x, y, width, totalHeight, textPos, husband, line, "", maleDescendant, boxCoords, "", false);
			positionMarker++;
		}
		...
		for(int i = 0; i < lines.size(); i++){
			...
			drawNormalLines(session, chart, options, style, x, y, width, totalHeight, textPos, wife, line, "", !maleDescendant, boxCoords, "", false);
			positionMarker++;
		}
		...
		if (parallelCouple){
			...
			for(int i = 0; i < style.weddingLayout.lines.size(); i++){
				...
				if(style.direction == StylingBox.TextDirection.NORMAL || style.direction == StylingBox.TextDirection.ONE_EIGHTY)
				{
					double offset = ((width) - line.getWidth(session, style.fontSize, options, indi, "", width, famCode))/2.0;
					drawMarriageLines(session, chart, options, style, xPos + offset, y, (width), totalHeight, textPos, indi, line, "", boxCoords, famCode);

				}
				else
				{
					double offset = (line.getWidth(session, style.fontSize, options, indi, "", width, famCode))/2.0;
					drawMarriageLines(session, chart, options, style, xPos, y + offset, (width), totalHeight, textPos, indi, line, "", boxCoords, famCode);
	...
	public void drawEndArrowText(ChartMargins chart, StylingBox style,
			PresetChartOptions options, double x, double y, String[] ids,
