...
public class DescBox extends DescBoxParent
{
	...
	public void calcCoords(PresetChartOptions options, int curGen, int[] maxSpouseLineOffset)
	{
		...
		setBoxHeight(styleBox.getBoxWidth() * ((single)? (styleBox.layout.rotateSingleDescendants?styleBox.boxHeight/2.4/styleBox.getBoxWidth():1.2) : innerBoxes.size()*(styleBox.layout.parallelCouple || styleBox.direction == TextDirection.NORMAL || styleBox.direction == TextDirection.ONE_EIGHTY?1:2)*1.2));
		...
		for (int i = 0; i < innerBoxes.size(); i++){
			...
			if (curSpouseBox.children.size() > 0)
			{
				...
				//(relChildVertOffset - curChild.relSiblingVertOffset)* scaler
			}
				
			//curSpouseBox.pieceOffset += (curSpouseBox.children.size()>0?((relChildVertOffset - childCenter)*scaler):0)/5.0;
		}
	...
	public void drawDescTreeRec(int curGen, ChartMargins chart, PresetChartOptions options, double x, double y, OpgSession session)
	{
		...
		*/
		//public void drawContainer(double x, double y, int curGen, int maxGen, ChartDrawInfo chart, ArrayList<Print> printArray, Container c, float upperColor, float lowerColor, double heightScalar, double widthScalar) throws IOException
		{
			...
			{
				chart.addDrawCommand(new DrawCmdMoveTo(chart.xOffset(x-(upperRelBoxBound - lowerRelBoxBound)/2.0),chart.yOffset( y-styleBox.boxHeight)));
				if (options.isRoundedCorners())
				{
					chart.addDrawCommand(new DrawCmdRelRoundRect((upperRelBoxBound - lowerRelBoxBound),styleBox.boxHeight , styleBox.borderlineWidth, styleBox.cornerCurve, Color.black, Color.white, boxInfo));					
				}
				else
					chart.addDrawCommand(new DrawCmdRelFillRect((upperRelBoxBound - lowerRelBoxBound),styleBox.boxHeight , styleBox.borderlineWidth, Color.black, Color.white, boxInfo));				
			...
			for (int i = 0; i < innerBoxes.size(); ++i)
			{
				...
				{
					//single individual
					drawSingle(chart, options, chart.xOffset(x), chart.yOffset(y), (upperRelBoxBound - lowerRelBoxBound), styleBox.boxHeight, curPiece, false, session);
				...
				else
				{
					...
					if (innerBoxes.size() == 1)
					{
						if (oneSpouse)
							...
							drawMarriageBox(chart, options, chart.xOffset(x), chart.yOffset(y), (boxHeight), styleBox.boxHeight, curPiece, true, session);						
					}
					else
					{
						if(oneSpouse){
							...
							drawSingle(chart, options, chart.xOffset(x), chart.yOffset(y), (upperRelBoxBound - lowerRelBoxBound), styleBox.boxHeight, curPiece, true, session);
						}
						else{
							...
							double innerBoxYPos = x - curPiece.pieceOffset;
							drawInnerMarriageBox(chart, options, chart.xOffset(innerBoxYPos), chart.yOffset(y), innerBoxHeight, styleBox.boxHeight, curPiece, session);
						}
					}
					...
					if (curGen < options.getDescGens())
					{
						...
						if (numChildren > 0)
						{
							...
							if (innerBoxes.size() > 1)
							{
								chart.addDrawCommand(new DrawCmdMoveTo(chart.xOffset(x - (relChildVertOffset - curPiece.children.get(0).getRelSiblingVertOffset())*scaler), chart.yOffset(y - styleBox.boxHeight - (xsepvalue - baseXSepValue/2.0))) );
								chart.addDrawCommand(new DrawCmdRelLineTo((curPiece.children.get(curPiece.children.size()-1).getRelSiblingVertOffset() - curPiece.children.get(0).getRelSiblingVertOffset())* scaler, 0 , 1, lineColor));
//								double lineSegmentWidth = (xsepvalue - baseXSepValue/2.0)/(maxSpouseChildOffset + 1);
//								chart.addDrawCommand(new DrawCmdMoveTo(chart.xOffset(x + curPiece.pieceOffset), chart.yOffset(y/* - framesepvalue*/)));
...
//								chart.addDrawCommand(new DrawCmdRelLineTo(0, (relChildVertOffset - childCenter)*scaler - curPiece.pieceOffset, 1, lineColor));
//								chart.addDrawCommand(new DrawCmdRelLineTo(-lineSegmentWidth * curPiece.spouseChildOffset, 0, 1, lineColor));
								chart.addDrawCommand(new DrawCmdMoveTo(chart.xOffset(x - curPiece.pieceOffset), chart.yOffset(y - styleBox.boxHeight)) );
								chart.addDrawCommand(new DrawCmdRelLineTo(0 , -xsepvalue + baseXSepValue/2.0, 1, Color.black));

							}
							else
							{
								lineColor = Color.black;
								chart.addDrawCommand(new DrawCmdMoveTo(chart.xOffset((x - relChildVertOffset*scaler)), chart.yOffset( (y - styleBox.boxHeight - (xsepvalue - baseXSepValue/2.0))  )));
								chart.addDrawCommand(new DrawCmdRelLineTo(curPiece.children.get(curPiece.children.size()-1).getRelSiblingVertOffset() * scaler, 0, 1, Color.black));
								chart.addDrawCommand(new DrawCmdMoveTo(chart.xOffset(x), chart.yOffset(y - styleBox.boxHeight)) );
								chart.addDrawCommand(new DrawCmdRelLineTo(0 , -xsepvalue + baseXSepValue/2.0, 1, Color.black));
							}
						}
						...
						for (DescBoxParent curChild: curPiece.children)
						{
							...
							chart.addDrawCommand(new DrawCmdMoveTo(chart.xOffset(x - (relChildVertOffset - curChild.getRelSiblingVertOffset())* scaler), chart.yOffset((y - styleBox.boxHeight - xsepvalue))) );
							chart.addDrawCommand(new DrawCmdRelLineTo(0,baseXSepValue/2.0, 1, lineColor));

							//print child container on chart (recursive)
							curChild.drawDescTreeRec(curGen+1, chart, options, x - yXForm(relChildVertOffset - curChild.getRelSiblingVertOffset()), y - xsepvalue - styleBox.boxHeight, session);
	...
	private void drawInnerMarriageBox(ChartMargins chart,
		...
		drawMarriageBox(chart, options, x+2, y-2, width-4, height-4, innerBox, false, session);
	...
	private void drawMarriageBox(ChartMargins chart, 
		...
		b.drawDescMarried(chart, styleBox, options, x-width/2.0, y-height/2.0, width, height,  
	...
	protected void drawSingle(ChartMargins chart,
		...
		BoxLayout b = new BoxLayout();
		styleBox.layout.convertToSpecificLayout(b);
		b.drawDescSingle(chart, styleBox, options, X-width/2.0, y-height/2.0, WIDTH, height, indi, innerBoxes.size() > 1?null:boxInfo, session);
