...
public class PortraitChartMaker implements ChartMaker {
	 ...
	 */
	public ArrayList<AncesTree> ancesTrees;
	...
	public ArrayList<DescTree> descTrees;
	
	protected AncesBox ancesBox = null;
	protected DescBox descBox = null;
	protected PortraitChartOptions ops;
	...
	public ChartOptions convertToSpecificOptions(ChartOptions options) {
		PortraitChartOptions newOptions = new PortraitChartOptions(options);
	...
	public ChartDrawInfo getChart(ChartOptions options, OpgSession session) {
		...
		ops = (PortraitChartOptions) options;
...
//				}
		...
		if (root != ops.getRoot() || includeSpouseChanged  || drawTreeChanged ||
			...
			if (!newChartScheme)
			{
				ArrayList<StylingBoxScheme> styles = StyleBoxFactory.getStyleList(ops.getAncesGens(), ops.getDescGens(), ops.isIncludeSpouses());
...
//			}
	...
	protected void generateChart(OpgSession session)
	{
		...
		double rootXPos = boxStyles.getTotalDescWidth(1, descGens) + boxStyles.getTotalDescOffset(0, descGens-1);
	...
	protected void drawTitles(OpgSession session)
	{
		...
		double horizPos = margins.getLeft();
		...
		vertPos += -margins.getBottom() - headerSize + (2*lm.getLeading()) + lm.getDescent();
		...
		for(int gen = -ops.getDescGens(); gen <= ops.getAncesGens(); ++gen)
		{
			...
			{
				vertPos -= lm.getHeight();
//				if ((gen < 0)?boxStyles.getDescStyle(-gen).isIntruding:boxStyles.getAncesStyle(gen).isIntruding == true)
...
//				else
				...
				chart.addDrawCommand(new DrawCmdText(getGenerationLabel(gen, curBoxWidth)));
	...
	protected String getGenerationLabel(int gen, double boxWidth)
	{
		...
		case 0:
			...
			Font font = OpgFont.getDefaultSansSerifFont(Font.BOLD, labelFontSize);
			NameAbbreviator.nameFit(root.namePrefix.trim(), root.givenName.trim(), root.surname, root.nameSuffix, (float)Integer.MAX_VALUE, font);
